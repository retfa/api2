...
@add_to_dict
class FTA_PM21_consumption_reel(Base):
    __tablename__ = 'FTA_PM21_consumption_reel'
    ...
    per_reel: Mapped[float] = mapped_column(DECIMAL)
    per_ton: Mapped[float] = mapped_column(DECIMAL)
    category: Mapped[str] = mapped_column(String(255))


class FTA_PM21_consumption_reelDal:
    def __init__(self):
    ...
    def query(self, filter):
        try:
            ...
            sqlcmd = f"""
SELECT relno, ptype, gramg, per_reel, per_ton, category
FROM [FTA_EN_Summary].[dbo].[FTA_PM21_consumption_reel]
            """
            if filter_conditions:
                sqlcmd += " WHERE " + " AND ".join(filter_conditions)
            sqlcmd += " ORDER BY etime"
            ic(sqlcmd)
