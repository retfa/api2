...
from DAL.srvmsdba2.TableBaseClass import RawTableBase_20
...
@add_to_dict
class PM20_CoatingWeight_raw(Base, RawTableBase_20):
    __tablename__ = 'PM20_CoatingWeight_raw'


class PM20_CoatingWeight_rawDal:
    def __init__(self):
    ...
    def query(self, filter):
        try:
            ...
            if filter.get('DtFrom') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.fta_dtm >= filter['DtFrom'])

            if filter.get('DtTo') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.fta_dtm < filter['DtTo'])

            print(len(filter.get('ReelNo')))
            if filter.get('ReelNo') is not None and len(filter.get('ReelNo')) > 0:
                # filter_conditions.append(PM20_CoatingWeight_raw.relno in (filter['ReelNo']))
                filter_conditions.append(PM20_CoatingWeight_raw.relno.in_(tuple(filter['ReelNo'])))

            if filter.get('ptype') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.ptype == filter['ptype'])

            if filter.get('gramg') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.gramg == filter['gramg'])

            final_filter_condition = and_(*filter_conditions)
            query = session.query(PM20_CoatingWeight_raw).filter(final_filter_condition)
    ...
    def select(self, filter):
        try:
            ...
            if filter.get('user_id') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.user_id == filter['user_id'])

            if filter.get('progm_id') is not None:
                filter_conditions.append(PM20_CoatingWeight_raw.progm_id == filter['progm_id'])

            filter_conditions.append(PM20_CoatingWeight_raw.IsEnabled == 1)
            final_filter_condition = and_(*filter_conditions)
            query = session.query(PM20_CoatingWeight_raw).filter(final_filter_condition)
            result_df = pd.read_sql(query.statement, session.bind)
            datetime_columns = result_df.select_dtypes(include=['datetime64[ns]']).columns
