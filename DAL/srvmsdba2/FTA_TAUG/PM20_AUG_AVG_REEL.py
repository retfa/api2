...
@add_to_dict
class PM20_AUG_AVG_REEL(Base):
    __tablename__ = 'PM20_AUG_AVG_REEL'
...
class PM20_AUG_AVG_REELDal:
    ...
    def query(self, filter):
        try:
            ...
            #     filter_conditions.append(PM20_AUG_AVG_REEL.fta_dtm >= filter['DtFrom'])

            # if filter.get('DtTo') is not None:
            #     filter_conditions.append(PM20_AUG_AVG_REEL.fta_dtm < filter['DtTo'])
            ...
            if filter.get('ReelNo') is not None and len(filter.get('ReelNo')) > 0:
                ...
                filter_conditions.append(PM20_AUG_AVG_REEL.relno.in_(tuple(filter['ReelNo'])))

            if filter.get('PaperCode') is not None:
                filter_conditions.append(PM20_AUG_AVG_REEL.ptype == filter['PaperCode'])

            if filter.get('BaseWeight') is not None:
                filter_conditions.append(PM20_AUG_AVG_REEL.gramg == filter['BaseWeight'])

            if filter.get('Category') is not None:
                filter_conditions.append(PM20_AUG_AVG_REEL.Category == filter['Category'])

            final_filter_condition = and_(*filter_conditions)
            query = session.query(PM20_AUG_AVG_REEL).filter(final_filter_condition)
