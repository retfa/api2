...
@add_to_dict
class BWRL_raw_avg(Base, RawTableBase_20):
    __tablename__ = 'BWRL_raw_avg'


class BWRL_raw_avgDal:
    def __init__(self):
    ...
    def query(self, filter):
        try:
            ...
            if filter.get('DtFrom') is not None:
                filter_conditions.append(BWRL_raw_avg.fta_dtm >= filter['DtFrom'])

            if filter.get('DtTo') is not None:
                filter_conditions.append(BWRL_raw_avg.fta_dtm < filter['DtTo'])

            print(len(filter.get('ReelNo')))
            if filter.get('ReelNo') is not None and len(filter.get('ReelNo')) > 0:
                # filter_conditions.append(BWRL_raw_avg.relno in (filter['ReelNo']))
                filter_conditions.append(BWRL_raw_avg.relno.in_(tuple(filter['ReelNo'])))

            if filter.get('ptype') is not None:
                filter_conditions.append(BWRL_raw_avg.ptype == filter['ptype'])

            if filter.get('gramg') is not None:
                filter_conditions.append(BWRL_raw_avg.gramg == filter['gramg'])

            final_filter_condition = and_(*filter_conditions)
            query = session.query(BWRL_raw_avg).filter(final_filter_condition)
    ...
    def select(self, filter):
        try:
            ...
            if filter.get('user_id') is not None:
                filter_conditions.append(BWRL_raw_avg.user_id == filter['user_id'])

            if filter.get('progm_id') is not None:
                filter_conditions.append(BWRL_raw_avg.progm_id == filter['progm_id'])

            filter_conditions.append(BWRL_raw_avg.IsEnabled == 1)
            final_filter_condition = and_(*filter_conditions)
            query = session.query(BWRL_raw_avg).filter(final_filter_condition)
            result_df = pd.read_sql(query.statement, session.bind)
            datetime_columns = result_df.select_dtypes(include=['datetime64[ns]']).columns
