...
@add_to_dict
class BW1RL_avg(Base, GroupTableBase_21):
    __tablename__ = 'BW1RL_avg'
    ...
    def query(self, filter):
        try:
            ...
            if filter.get('DtFrom') is not None:
                filter_conditions.append(BW1RL_avg.fta_dtm >= filter['DtFrom'])

            if filter.get('DtTo') is not None:
                filter_conditions.append(BW1RL_avg.fta_dtm < filter['DtTo'])
            ...
            if filter.get('ReelNo') is not None and len(filter.get('ReelNo')) > 0:
                ...
                filter_conditions.append(BW1RL_avg.relno.in_(tuple(filter['ReelNo'])))

            if filter.get('ptype') is not None:
                filter_conditions.append(BW1RL_avg.ptype == filter['ptype'])

            if filter.get('gramg') is not None:
                filter_conditions.append(BW1RL_avg.gramg == filter['gramg'])
